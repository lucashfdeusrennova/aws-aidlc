...
const fileRaw: string = parsed.tool_input?.file_path ?? "";
// Windows workaround: Kiro IDE on Windows sometimes hands us the file path with
// a lowercase drive letter, while Node's own `resolve`/`realpath` returns the
// OS-canonical (uppercase) drive letter for the same path. The audit `File`
// field is later compared verbatim to `resolvePath(<relative artifact path>)`
// (see verifyPreApprovalSummaryConfirmation in aidlc-lib.ts), so a lowercase
// drive letter causes a legitimate write to be treated as "no recorded native
// write" and blocks the approval gate. Canonicalize the drive letter here so
// the emitted `File` matches the resolver's canonical output.
const file: string = process.platform === "win32" && /^[a-zA-Z]:[\\/]/.test(fileRaw)
  ? fileRaw.charAt(0).toUpperCase() + fileRaw.slice(1)
  : fileRaw;
...
// Windows workaround: Kiro IDE sometimes reports file paths with a lowercase
// drive letter while `docsRoot(projectDir)` uses the OS-canonical (often
// uppercase) drive letter. A case-sensitive string comparison then treats a
// legitimate write to the record root as external, and the hook exits without
// emitting ARTIFACT_CREATED/ARTIFACT_UPDATED. Normalize the drive-letter case
// (and only that) for the path-root comparison; the emitted `File` field
// keeps the original casing so downstream tools resolve paths unchanged.
const normalizeDriveCase = (p: string): string =>
  process.platform === "win32" && /^[a-zA-Z]:\//.test(p)
    ? p.charAt(0).toLowerCase() + p.slice(1)
    : p;
const fileNormForRoot = normalizeDriveCase(fileNorm);
const recordRootForCmp = normalizeDriveCase(recordRoot);
const underRecord = fileNormForRoot === recordRootForCmp || fileNormForRoot.startsWith(`${recordRootForCmp}/`);
const codekbRoot = join(codekbDir(projectDir, "_"), "..")
  .replace(/\\/g, "/")
  .replace(/\/$/, "");
const codekbRootForCmp = normalizeDriveCase(codekbRoot);
const underCodekb = fileNormForRoot.startsWith(`${codekbRootForCmp}/`);
